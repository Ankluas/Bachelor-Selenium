...
for(i in sub.folders){
  ...
  if((file.exists("knockoutList.txt")) && ((file.size("knockoutList.txt")) > 0)) {
    if(!exists("dataset")){
      dataset <- data.table(read.table("knockoutList.txt"))
      print(dataset)
    }else{
      data <-data.table(read.table("knockoutList.txt"))
      print(data)
...
# calc. average number of websites that use knockout
