...
#dataset2 <- dataset[,list(src=mean(srcextern)),by=name]

# Prozente
dataset
dataset[,sum(httpEinbindung)/sum(jstype)]
dataset[,sum(httpsEinbindung)/sum(jstype)]
dataset[,sum(jstype)/sum(scriptcount)]
dataset[,sum(srcextern)/sum(scriptcount)]
dataset[,sum(async)/sum(srcextern)]
dataset[,sum(httpEinbindung)/sum(srcextern)]
dataset[,sum(httpsEinbindung)/sum(srcextern)]
# rest is per http

# fix JSs too small
#dataset[,JSs:=max(jstype,httpEinbindung+httpsEinbindung),by=list(number,V9)]
# change
dataset2 <- dataset[jstype!=0,list(meanratiohttpssrc=mean(httpsEinbindung/jstype)),by=name]
#change
ggplot(dataset2,aes(x=name,y=meanratiohttpssrc*100)) +
  geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  xlab("website sections") +
  ylab("percentage")