...
for(i in sub.folders){
  ...
  if((file.exists("backboneList.txt")) && ((file.size("backboneList.txt")) > 0)) {
    if(!exists("dataset")){
      dataset <- data.table(read.table("backboneList.txt"))
      print(dataset)
    }else{
      data <-data.table(read.table("backboneList.txt"))
      print(data)
...
# calc. average number of websites that use backbone
