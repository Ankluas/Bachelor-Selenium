...
for(i in sub.folders){
  ...
  if((file.exists("angularList.txt")) && ((file.size("angularList.txt")) > 0)) {
    if(!exists("dataset")){
      dataset <- data.table(read.table("angularList.txt"))
      print(dataset)
    }else{
      data <-data.table(read.table("angularList.txt"))
      print(data)
...
# calc. average number of websites that use angular
