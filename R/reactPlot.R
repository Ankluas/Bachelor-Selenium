...
for(i in sub.folders){
  ...
  if((file.exists("reactList.txt")) && ((file.size("reactList.txt")) > 0)) {
    if(!exists("dataset")){
      dataset <- data.table(read.table("reactList.txt"))
      print(dataset)
    }else{
      data <-data.table(read.table("reactList.txt"))
      print(data)
...
# calc. average number of websites that use react
